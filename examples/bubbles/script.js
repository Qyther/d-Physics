...
window.onload=setTimeout(()=>{
    ...
    if(!last) last=now;
    var delta=now-last;
    for(var i=0;i<box.poin.length-2;i++) {
        for(var j=0;j<box.poin.length;j++) {
            if(i!==j)
                box.poin[i].resolveCollision(box.poin[j],(box.poin[i].radi+box.poin[j].radi)*1.1,1);
        }
    ...
    ctx.strokeStyle="#222";
    ctx.beginPath();
    ctx.arc(mp0.posi[0],mp0.posi[1],mp0.radi*1.5,0,2*Math.PI);
    ctx.stroke();
    ctx.closePath();
